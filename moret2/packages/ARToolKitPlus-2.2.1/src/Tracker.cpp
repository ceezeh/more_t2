...
Tracker::Tracker(int imWidth, int imHeight, int maxImagePatterns, int pattWidth, int pattHeight, int pattSamples, int maxLoadPatterns) :
		...
		evecBW(EVEC_MAX, vector<ARFloat>(PATTERN_HEIGHT*PATTERN_WIDTH*3))
 {
	...
	// init
	memset( workL, 0, sizeof(int) * WORK_SIZE );
	memset( work2L, 0, sizeof(int) * WORK_SIZE  * 7 );
	memset( wareaL, 0, sizeof(int) * WORK_SIZE );
	memset( wclipL, 0, sizeof(int) * WORK_SIZE *4 );
	memset( wposL, 0, sizeof(ARFloat) * WORK_SIZE *2 );
