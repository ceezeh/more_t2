...
inline
void floatToFixed(ARFloat nX, ARFloat nY, LUT_Type &nFixed) {
#ifdef OLD
...
inline
void fixedToFloat(LUT_Type nFixed, ARFloat& nX, ARFloat& nY) {
#ifdef OLD
...
void Tracker::buildUndistO2ITable(Camera* pCam) {
    ...
    undistO2ITable = new LUT_Type[arImXsize * arImYsize];

    if (loadCachedUndist) {
        if (FILE* fp = fopen(cachename, "rb")) {
            size_t numBytes = fread(undistO2ITable, 1, arImXsize * arImYsize * sizeof(LUT_Type), fp);
            fclose(fp);

            if (numBytes == arImXsize * arImYsize * sizeof(LUT_Type))
                loaded = true;
        }
    }
    ...
    if (!loaded) {
        ...
        if (loadCachedUndist)
            if (FILE* fp = fopen(cachename, "wb")) {
                fwrite(undistO2ITable, 1, arImXsize * arImYsize * sizeof(LUT_Type), fp);
                fclose(fp);
            }
    }
