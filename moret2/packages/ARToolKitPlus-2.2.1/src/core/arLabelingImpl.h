								  ...
								  ARFloat **pos, int **clip, int **label_ref)
{
	...
	if(pixelFormat!=PIXEL_FORMAT_RGB565 && pixelFormat!=PIXEL_FORMAT_LUM && pixelFormat!=PIXEL_FORMAT_YUY2)
		thresh *= 3;
...
//		difftopBottom = -20;
	...
	int threshFact = (pixelFormat!=PIXEL_FORMAT_RGB565 && pixelFormat!=PIXEL_FORMAT_LUM && pixelFormat!=PIXEL_FORMAT_YUY2) ? 3 : 1;
...
#endif
#ifdef _DEF_PIXEL_FORMAT_YUY2
				isBlack = (getLUM8_from_YUY2(pnt) <= corrThresh );
#endif
			...
			if(isBlack) {
                ...
                if( *pnt1 > 0 ) {
                    ...
                    *pnt2 = *(pnt2-1);
					...
					//assert( wk_max >= -32768 && wk_max <= 32767 );
                    *pnt2 = wk_max; /// BUG !!!
