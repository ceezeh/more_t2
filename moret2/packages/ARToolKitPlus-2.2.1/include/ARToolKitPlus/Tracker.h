...
protected:
    ...
    ARMarkerInfo2* arDetectMarker2(int/*16_t*/ *limage, int label_num, int *label_ref, int *warea, ARFloat *wpos,
            int *wclip, int area_max, int area_min, ARFloat factor, int *marker_num);

    int arGetContour(int/*16_t*/ *limage, int *label_ref, int label, int clip[4], ARMarkerInfo2 *marker_infoTWO);
    ...
    int16_t* arLabeling_LUM(uint8_t *image, int thresh, int *label_num, int **area, ARFloat **pos, int **clip,
            int **label_ref);
	int16_t* arLabeling_YUY2(uint8_t *image, int thresh, int *label_num, int **area, ARFloat **pos, int **clip,
            int **label_ref);
#else
	int* arLabeling(uint8_t *image, int thresh, int *label_num, int **area, ARFloat **pos, int **clip,
        int **label_ref);

    int* arLabeling_ABGR(uint8_t *image, int thresh, int *label_num, int **area, ARFloat **pos, int **clip,
        int **label_ref);
    int* arLabeling_BGR(uint8_t *image, int thresh, int *label_num, int **area, ARFloat **pos, int **clip,
        int **label_ref);
    int* arLabeling_RGB(uint8_t *image, int thresh, int *label_num, int **area, ARFloat **pos, int **clip,
        int **label_ref);
    int* arLabeling_RGB565(uint8_t *image, int thresh, int *label_num, int **area, ARFloat **pos, int **clip,
        int **label_ref);
    int* arLabeling_LUM(uint8_t *image, int thresh, int *label_num, int **area, ARFloat **pos, int **clip,
        int **label_ref);
	int* arLabeling_YUY2(uint8_t *image, int thresh, int *label_num, int **area, ARFloat **pos, int **clip,
            int **label_ref);
#endif 
...
protected:
    struct AutoThreshold {
        ...
        void addValue(int nRed, int nGreen, int nBlue, int nPixelFormat) {
            ...
            // in RGB565 and LUM8 and YUY2 all three values are simply the grey value...
            if (nPixelFormat == PIXEL_FORMAT_RGB565 || nPixelFormat == PIXEL_FORMAT_LUM || nPixelFormat == PIXEL_FORMAT_YUY2)
                lum = nRed;
            else
    ...
    //int16_t *l_imageR;
    int *l_imageL; //[HARDCODED_BUFFER_WIDTH*HARDCODED_BUFFER_HEIGHT];		// dyna
    int *l_imageR;
    int l_imageL_size;
