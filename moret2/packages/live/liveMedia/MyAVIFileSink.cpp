...
enum state {NB_ENABLE, NB_DISABLE};
...
public:
  AVISubsessionIOState(MyAVIFileSink& sink, MediaSubsession& subsession);
...
////////// MyAVIFileSink implementation //////////

MyAVIFileSink::MyAVIFileSink(UsageEnvironment& env,
			 MediaSession& inputSession,
			 char const* outputFilePrefix, Boolean triggered_t,
			 unsigned bufferSize,
			 unsigned short movieWidth, unsigned short movieHeight,
			 unsigned movieFPS, Boolean packetLossCompensate)
  : Medium(env), fInputSession(inputSession), triggered(triggered_t),
    ...
    mkdir(outputFilePrefix, 0775);
    stringstream name;
    //open timestamp file.
    name << outputFilePrefix << "/timestamps.txt";
    timestampFile.open(name.str().c_str(), std::ofstream::out|std::ofstream::trunc);
    name.str("");
    // open Triggering file.
    if (triggered) {
      nonblock(NB_ENABLE);
      name << outputFilePrefix << "/triggers.txt";
      triggerFile.open(name.str().c_str(), std::ofstream::out|std::ofstream::trunc);
      if (!triggerFile.is_open() ) return;
    }
    // open video file.
    stringstream outputFilename;
    outputFilename << outputFilePrefix << "/vid.avi";
    fOutFid = OpenOutputFile(env, outputFilename.str().c_str());
    if (!timestampFile.is_open() ) return;
...
::createNew(UsageEnvironment& env, MediaSession& inputSession,
	    char const* outputFilePrefix, Boolean trig,
	    unsigned bufferSize,
	    unsigned short movieWidth, unsigned short movieHeight,
	    unsigned movieFPS, Boolean packetLossCompensate) {
  MyAVIFileSink* newSink =
    new MyAVIFileSink(env, inputSession, outputFilePrefix, trig, bufferSize,
...
Boolean MyAVIFileSink::startPlaying(afterPlayingFunc* afterFunc,
...
Boolean MyAVIFileSink::continuePlaying() {
...
::afterGettingFrame(void* clientData, unsigned packetDataSize,
  ...
  if (numTruncatedBytes > 0) {
    ioState->envir() << "MyAVIFileSink::afterGettingFrame(): The input frame data was too large for our buffer.  "
		     << numTruncatedBytes
...
void MyAVIFileSink::onSourceClosure(void* clientData) {
  AVISubsessionIOState* ioState = (AVISubsessionIOState*)clientData;
  ioState->onSourceClosure();
}

void MyAVIFileSink::onSourceClosure1() {
...
void MyAVIFileSink::onRTCPBye(void* clientData) {
  AVISubsessionIOState* ioState = (AVISubsessionIOState*)clientData;
...
void MyAVIFileSink::addIndexRecord(AVIIndexRecord* newIndexRecord) {
...
void MyAVIFileSink::completeOutputFile() {
...
AVISubsessionIOState::AVISubsessionIOState(MyAVIFileSink& sink,
				     MediaSubsession& subsession)
...
void AVISubsessionIOState::useFrame(SubsessionBuffer& buffer) {
  ...
  struct timeval tp;
    ...
    timestamp_t now = (long long) tp.tv_sec * 1000L + tp.tv_usec / 1000; //get current timestamp in milliseconds

  // Add presentation time to timefile.
  // timestamp_t now = (timestamp_t)(presentationTime.tv_sec *1000000 + presentationTime.tv_usec);


 // Add trigger if available.
  if (fOurSink.triggered) {
    if (!fOurSink.triggerFile.is_open()) exit(1);
        fd_set readset;
        struct timeval tv;
        tv.tv_sec = 0;
        tv.tv_usec = 100;
        ...
        if(FD_ISSET(fileno(stdin), &readset))
        {
          char c = fgetc (stdin);
           if (c == '1') {
            fOurSink.triggerFile << now  << "\n";
            fOurSink.triggerFile.flush();

          }
        }
      }

 if (!fOurSink.timestampFile.is_open()) exit(1);
  fOurSink.timestampFile << now  << "\n";
  fOurSink.timestampFile.flush();
...
unsigned MyAVIFileSink::addWord(unsigned word) {
...
unsigned MyAVIFileSink::addHalfWord(unsigned short halfWord) {
...
unsigned MyAVIFileSink::addZeroWords(unsigned numWords) {
...
unsigned MyAVIFileSink::add4ByteString(char const* str) {
...
void MyAVIFileSink::setWord(unsigned filePosn, unsigned size) {
  do {
  ...
  envir() << "MyAVIFileSink::setWord(): SeekFile64 failed (err "
	  << envir().getErrno() << ")\n";
}
...
#define addFileHeader(tag,name) \
    unsigned MyAVIFileSink::addFileHeader_##name() { \
        add4ByteString("" #tag ""); \
