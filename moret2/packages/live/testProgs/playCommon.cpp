  ...
  while (argc > 1) {
	...
	*env << "bad port number: " << portArg
    ...
    case 'i': { // output an AVI file (to stdout)
     ...
     case 'x': { // camera id (to stdout)
      if (sscanf(argv[2], "%hd", &id) != 1) {
  usage();
      }
      ++argv; --argc;
      break;
	...
	*env << "offset argument to \"-S\" must be >= 0\n";
    ...
    case 'L': { // specify a full name for the audio and video output files
      avifileName = argv[2];
      ++argv; --argc;
      break;
    }
    ...
    case 'e': { // enable triggering via input command.
      triggering = true;
    }
  ...
  if (outputQuickTimeFile || outputAVIFile) {
    if (periodicFilenameSuffix[0] == '\0') {
      ...
      sprintf(outFileName, "stdout");
        ...
        sprintf(outFileName, "%s/vid.avi", avifileName);
      }
    } else {
	...
	*env << "Outputting to the file: \"" << outFileName << "\"\n";
      ...
      aviOut = MyAVIFileSink::createNew(*env, *session, avifileName, triggering,
				      fileSinkBufferSize,
				      movieWidth, movieHeight,
				      movieFPS,
				      packetLossCompensate);
      if (aviOut == NULL) {
	*env << "Failed to create an \"MyAVIFileSink\" for outputting to \""
	     << outFileName << "\": " << env->getResultMsg() << "\n";
	shutdown();
      } else {
	*env << "Outputting to the file: \"" << outFileName << "\"\n";
